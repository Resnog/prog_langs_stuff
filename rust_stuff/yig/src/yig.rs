...
const YIG_CONFIG_MSG_HEADER: &str = "YIG_CONFIG >>";
const YIG_ARCHIVE_MSG_HEADER: &str = "YIG_ARCHIVE >>";
...
pub mod config {
    ...
    pub fn init() {
        ...
        if yig_config_path.exists() {
            println!(
                "{} YIG config directory already exists at:\n    {}",
                YIG_CONFIG_MSG_HEADER,
                yig_config_path.display()
            );
        } else {
...
pub mod archive {
    ...
    pub fn init() {
        let archive_dir = format!(
            ...
            YIG_DEF_ARCHIVE_PATH
        );
        let archive_path = path::Path::new(&archive_dir);

        if archive_path.exists() {
            println!(
                "{} Archive directory already exists at:\n    {}",
                YIG_ARCHIVE_MSG_HEADER,
    ...
    pub fn load(filename: String) {
        ...
        if filepath.is_file() {
            ...
            println!(
                "{} File found in archive. Copying file.",
                YIG_ARCHIVE_MSG_HEADER
            );
            File::create(&filepath).expect("File creation failed.");
...
pub mod commands {
    ...
    use toml::{self, Table};
    const YIG_COMMANDS_MSG_HEADER: &str = "YIG COMMANDS >>";
    use super::get_config_dir;
    ...
    pub fn init() {
        ...
        if Path::new(&yig_commands_file).exists() {
            println!(
                "{} Commands file already exists at:\n    {}",
...
"#;
    ...
    pub fn call(command_name: &str) {
        ...
        match command {
            ...
            None => println!(
                "{} Command \'{}\' not found in commands file.",
                YIG_COMMANDS_MSG_HEADER, command_name
            ),
        };
    }
    ...
    fn get_commands_string() -> String {
        ...
        let commands =
            std::fs::read_to_string(yig_commands_file).expect("YIG commands file read failed.");
        commands
    }

    fn get_commands() -> toml::Value {
        let commands = get_commands_string();
        let commands_values: toml::Value =
            toml::from_str(&commands).expect("YIG commands file parse failed.");
    ...
    pub fn add() {
        ...
        println!(
            "{} Enter the new command name for you to call: ",
            YIG_COMMANDS_MSG_HEADER
        );
        ...
        commands_table.insert(
            new_user_cmd.trim().to_string(),
            toml::Value::String(new_exec_cmd.trim().to_string()),
        );
        ...
        println!(
            "{} path: {}",
            YIG_COMMANDS_MSG_HEADER, yig_commands_filename
        );
        let new_contents = toml::to_string(&main_table).unwrap();
        std::fs::write(yig_commands_filename, new_contents)
            .expect("YIG commands file write failed.");
    }
